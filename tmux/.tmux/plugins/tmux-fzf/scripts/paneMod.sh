...
if [[ "$action" == "layout" ]]; then
    ...
    elif [[ "$2" == "current" || "$action" == "transfer" ]]; then
        target_origin=$current_pane_origin
    else
    ...
    if [[ "$action" == "switch" ]]; then
        ...
        win_nrs=$(tmux list-windows)
        target_win=$(printf "$win_nrs" | eval "$TMUX_FZF_BIN $TMUX_FZF_OPTIONS $TMUX_FZF_PREVIEW_OPTIONS")
        echo "$target_win"
        echo "${target_win:0:1}"
        tmux join-pane -t "${target_win:0:1}"
    fi
fi
