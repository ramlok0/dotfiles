...
define semaphore_owner_uclibc
        if $argc == 0
		printf "semaphore_owner_uclibc <sem>\n"
	else
		set $my_thread_desc =  &__pthread_handles+(($arg0.m_owner % __linuxthreads_pthread_threads_max)*4)+2
		set  $my_thread_desc = *$my_thread_desc
		set  $my_thread_desc = (int *)$my_thread_desc
		printf "thread id \t= %u\n", $my_thread_desc[4]
		printf "thread tid \t= %u\n", $my_thread_desc[5]
        end
end

define semaphore_owner_glibc
        if $argc == 0
		printf "semaphore_owner_glibc <sem>\n"
	else
		printf "thread id \t= %u\n", $arg0.m_owner
		printf "thread tid \t= %u\n", ((pthread *)$arg0.m_owner)->tid
        end
end
	...
	if $argc == 0
		...
		if $argc == 2
			while $i < $tree_size
				...
				printf "elem[%u]: ", $i
				p *($arg1*)$value
				printf "Waiting thread tid[%u]: \n", ((struct pthread *) (*($arg1*)$value) )->tid
				if $waitingCount == 0
				    set $waitingThread0 = ((struct pthread *) (*($arg1*)$value) )->tid
				end
				if $waitingCount == 1
				    set $waitingThread1 = ((struct pthread *) (*($arg1*)$value) )->tid
				end
				if $waitingCount == 2
				    set $waitingThread3 = ((struct pthread *) (*($arg1*)$value) )->tid
				end
...
define semaphoreInfo
        if $argc == 0
		printf "semaphoreInfo <sem>\n"
	else
	    set $sema = $arg0
	    set $ownerTid = ((struct pthread *)$sema.m_owner)
	    print $ownerTid
	    printf "Owner Tid is:[%u] \n", ((struct pthread *)$ownerTid)->tid
	    set $ownerTid = ((struct pthread *)$ownerTid)->tid
	    pset $sema.m_waitingThreads GPAL_THREAD_ID
	    psetSema $sema.m_waitingThreads GPAL_THREAD_ID
	end

end


define semaphoreInfoOwner
        if $argc == 0
		printf "semaphoreInfo <sem>\n"
	else
	    set $sema = $arg0
